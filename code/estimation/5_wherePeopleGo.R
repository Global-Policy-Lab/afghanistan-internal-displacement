...
for (p in length(suffix):1) {
  outFileName <- paste0("/data/tmp/xtai/9-22wherePeopleGo/betaCoefList_", suffix[p], ".rds")
...
plot2 <-totalDisplacement %>%
  ...
  mutate(key = case_when(key == "top5exclhomeComp_capitals" ~ "Different province:\nTop-5 cities",
                         ...
                         TRUE ~ key)) %>%
  ggplot(aes(key, exp(estimate)), col = as.factor(tmpCol))+
  geom_pointrange(aes(ymin = exp(conf.low), ymax = exp(conf.high), lty = as.factor(daysAfter), col = as.factor(tmpCol)),
                  position = position_dodge(width = .4),
                  # size = .4, fatten = 2) +
                  size = 1.4, fatten = 2) + # fatten changes the circles only; size changes bars and legend circles
  coord_flip() + 
  ...
  guides(#color = guide_legend(override.aes = list(linetype = "blank")), 
         ...
         linetype = guide_legend(override.aes = list(shape = NA))) +  # shape for the type of dot
  # scale_x_discrete(labels = ) +
  theme(plot.title = element_text(size = 15, face = "bold"),
    legend.position= "top",
...
png(paste0("/data/tmp/xtai/9-22wherePeopleGo/plots/fig5.png"), width = 12, height = 7, res = 300, units = "in")
